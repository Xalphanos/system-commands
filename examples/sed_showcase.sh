...
# This uses an address range with a regular expression to keep matching lines.
sed -n '/ [7-9][0-9]{4}$/p' "$DATA_FILE"

print_section "Delete header comment lines and trailing blank lines"
# The sample file uses a leading comment to show sed deletion of anchors.
sed '/^#/d; /^$/d' "$DATA_FILE"

print_section "Numbered lines with custom formatting"
# Equivalent to `nl -ba` but implemented with sed capture groups.
sed = "$DATA_FILE" | sed 'N; s/\n/: '/

print_section "In-place edit example (writes to a temporary copy)"
# Always edit a copy so the original fixture is preserved.
=======
=======
>>>>>>> theirs
=======
