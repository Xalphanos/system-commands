...
# - Read the comments as a “what is happening and why” guide, then change the patterns to prove you understand them.
set -euo pipefail
...
# Try removing one of the substitutions or changing the replacement text to see how the output differs.
...
# Change the leading space to ^ to see how the regex matches the ID instead of the salary field.
...
# Add another pattern (for example, /Marketing/) to see how deletion rules stack.
sed '/^#/d; /^$/d' "$DATA_FILE"
...
# Replace ": " with " -> " or swap the order in s/\n/: / to understand how pattern space manipulation works.
...
# On macOS you may need to provide a backup suffix to -i, e.g. -i ''. To experiment safely, change the replacement prefix.
tmp_copy=$(mktemp)
